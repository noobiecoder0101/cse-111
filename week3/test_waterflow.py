...
from waterflow import pressure_loss_from_fittings
from waterflow import reynolds_number
from waterflow import pressure_loss_from_pipe_reduction
def test_water_column_height():
    assert isinstance(water_column_height(0.00,0.00), float )
...
def test_pressure_gain_from_water_height ():
    assert isinstance(pressure_gain_from_water_height(0.0), float)
    assert pressure_gain_from_water_height(0.00) == approx(0.000 ,abs=0.001)
    assert pressure_gain_from_water_height(30.2) == approx(295.628 ,abs=0.001)
    assert pressure_gain_from_water_height(50.0) == approx(489.450 ,abs=0.001)
def test_pressure_loss_from_pipe():
    assert isinstance(pressure_loss_from_pipe(0.04272, 0.00, 18.0, 1.75), float)
...
def test_pressure_loss_from_fittings():
    assert isinstance(pressure_loss_from_fittings(0.00, 3,), float)
...
def test_reynolds_number():
    assert isinstance(reynolds_number(0.048692, 0.00), float)
...
def test_pressure_loss_from_pipe_reduction():
    assert isinstance(pressure_loss_from_pipe_reduction(0.28687, 0.00, 1, 0.048692), float)
